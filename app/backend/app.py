...
async def create_app():
    ...
    openai_model = os.environ.get("OPENAI_REALTIME_MODEL", "gpt-4o-realtime-preview") # Add default

    if not openai_api_key:
        raise ValueError("OPENAI_API_KEY environment variable not set.")


    # --- Remove Azure credential creation logic ---
    # --- Remove Azure key loading (or keep for reference) ---
    ...
    rtmt = RTMiddleTier(
        ...
        model=openai_model,
        # Pass system_message and voice_choice directly if needed,
        # or set them after initialization as before.
        voice_choice=os.environ.get("AZURE_OPENAI_REALTIME_VOICE_CHOICE") or "alloy" # Reuse existing env var for voice
    )
